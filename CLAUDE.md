...
- `translate_only` → `run_translate_only` — same cache wipe before running, then stops after `translate_segments`; returns the translated-subtitle path when saved, else `transcription_path`. Both original and translated subtitles honor the corresponding flags.
